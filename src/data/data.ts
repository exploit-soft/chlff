...
export const generateQuestions = (
  ...
  const allQuestions = getAllQuestions(level);
  ...
  return allQuestions.filter((q) => {
    switch (operator) {
      case 'addition':
        return q.question.includes('+');
      case 'subtraction':
        return q.question.includes('-');
      case 'multiplication':
        return q.question.includes('x') || q.question.includes('×');
      case 'division':
        return q.question.includes('÷');
      default:
        return true;
...
const getAllQuestions = (level: Level): Question[] => {
  switch (level) {
    case Level.YEAR_1:
      return [
        { question: '1 + 1?', options: shuffleArray(['1', '2']), answer: '2' },
